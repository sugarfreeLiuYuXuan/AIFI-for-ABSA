...
class HI_ASA(nn.Module):
    def __init__(self, config,args):
        ...
        self.bert = BertModel(config)
        self.ate = DynamicLSTM(config.hidden_size, args.hidden_size, num_layers=2, batch_first=True, bidirectional=True, rnn_type='GRU')
        self.atc= DynamicLSTM(config.hidden_size, args.hidden_size, num_layers=2, batch_first=True, bidirectional=True, rnn_type='GRU')
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, mode, attention_mask, input_ids=None, token_type_ids=None, start_positions=None, end_positions=None, aspect_num=None,
        ...
        if mode == 'train':
            ...
            sequence_output_ate_init, hidden_ae = self.ate(sequence_output, L_tensor )  # [batch_size,seq_len, d]
            sequence_output_atc_init, hidden_ac = self.atc(sequence_output, L_tensor )  # [batch_size,seq_len,d]
            # get hidden          
            hidden_ae = hidden_ae[0].permute(1, 0, 2).view(batch_size, -1)
            hidden_ac = hidden_ac[0].permute(1, 0, 2).view(batch_size, -1)
           ...
           # shallow interactive
            ...
            # print(f"sequence_output_ate size: {sequence_output_ate.size()}" )  # (batch, max_len ,768)
            # print(f"sequence_output_atc size: {sequence_output_atc.size()}" )   # (batch, max_len ,768)

            # add L2 Regularization loss
            # reg_loss = F.mse_loss(hidden_ae, hidden_ac)
            # B, L, N = hidden_ac.shape
            # hidden_ac = torch.chunk(hidden_ac, L, dim=1)
            # hidden_ae = torch.chunk(hidden_ae, L, dim=1)
            # hidden_ac = torch.mean(torch.sum(torch.stack(hidden_ac), dim=0), dim=1, keepdim=True).squeeze(1)
            # hidden_ae = torch.mean(torch.sum(torch.stack(hidden_ae), dim=0), dim=1, keepdim=True).squeeze(1)
            
            # InfoNCE loss
            assert sequence_output_ate.size()[-1] == sequence_output_atc.size()[-1]
            infoNCE_loss = InfoNCE(input_dim=sequence_output_ate.size(), 
                                   scl_label=scl_label,
                                   max_temperature=max_temperature,
                                   min_temperature=min_temperature)
            # print(f"hidden_ac: {hidden_ac.size()}")
            if self.use_static_temperature:
                sim_loss = infoNCE_loss(hidden_ae, hidden_ac, sequence_output_ate, sequence_output_atc, use_static_temperature=self.use_static_temperature)
            else:
                sim_loss, at_loss = infoNCE_loss(hidden_ae, hidden_ac, sequence_output_ate, sequence_output_atc, use_static_temperature=self.use_static_temperature)
            ...
            span_output, span_mask = get_span_representation(span_starts,
                                                             span_ends, sequence_output_atc,
                                                             attention_mask)  # [N*M, JR, d], [N*M, JR]
            # print(f"span_output size: {span_output.size()}") #[10 ,2, 768]
            # print(f"span_mask size: {span_mask.size()}")    #[10 ,2]
            ...
            interaction_mat = torch.matmul(sequence_output_atc, torch.transpose(span_output, 1, 2))  #[N*M, L, JR]
                # print(f"interaction_mat size {interaction_mat.size()}")  #([10, 85, 2])
            alpha = torch.nn.functional.softmax(interaction_mat, dim=1)  #[N*M, L, JR] [10, 85, 2]
            beta = torch.nn.functional.softmax(interaction_mat, dim=2) #[N*M, L, JR] [10, 85, 2]
            beta_avg = beta.mean(dim=1, keepdim=True)   #[N*M, 1, JR] [10, 1, 2]
            gamma = torch.matmul(alpha, beta_avg.transpose(1, 2)) # [N*M, L, 1] [10, 85, 2] * [10, 2, 1] = [10, 85, 1]
            ...
            # mutual_loss = - js_div(aspect_score,gamma.squeeze(-1) )
            if self.use_static_temperature:
                return ae_loss + ac_loss + sim_loss
            if self.use_deep_share:
...
# if __name__ == "main":
#     kwargs = {'hidden_size':384, 
#               'shared_weight': 0.1,
#               'n_best_size': 10,
#               'weight_kl':0.1,
#               'logit_threshold':0.5}
    
#     args = testArgs(**kwargs)
#     bert_config = BertConfig.from_json_file("cache_bert_large/config.json")
#     model = HI_ASA(bert_config,args)
