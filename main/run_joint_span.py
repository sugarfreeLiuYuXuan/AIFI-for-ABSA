...
from main.utils import read_absa_data, convert_absa_data, convert_examples_to_features, RawFinalResult, RawSpanResult,  span_annotate_candidates_lin
from main.run_base import copy_optimizer_params_to_model, set_optimizer_params_grad, prepare_optimizer, post_process_loss, bert_load_state_dict
from main.run_cls_span import eval_absa,eval_ac
...
def read_train_data(args, tokenizer, logger):
    ...
    # add scl_label
    all_scl_label = torch.tensor([f.scl_label for f in train_features], dtype=torch.float)
    
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_start_positions, all_end_positions,
                               all_aspect_num, all_example_index, all_scl_label)
...
def run_train_epoch(args, global_step, model, param_optimizer,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        # need scl_token
        input_ids, input_mask, segment_ids, start_positions, end_positions, aspect_num, example_indices, scl_label = batch
        
        batch_start_logits, batch_end_logits,batch_aspect_num, _ = model('extract_inference', 
                                                                         input_mask, 
                                                                         input_ids=input_ids, 
...
def main():
    ...
    parser.add_argument("--bert_config_file", default='bert-large-uncased/config.json', type=str, required = True)
