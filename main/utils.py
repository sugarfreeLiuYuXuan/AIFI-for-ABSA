...
class SemEvalExample(object):
    ...
    def __repr__(self):
        ...
        if self.scl_label:
            s += ", scl_label: {}".format(self.scl_label)
        return s
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, verbose_logging=False, logger=None):
    ...
    for (example_index, example) in enumerate(examples):
        ...
        if example_index < 1 and verbose_logging:
            ...
            logger.info("scl_label: {}".format(scl_label))
...
def convert_absa_data(dataset, verbose_logging=False):
    ...
    for i in range(n_records):
        ...
        if term_texts != []:
            ...
            # add scl_label
            scl_label = len(list(filter(lambda x: x!="neutral", new_polarities)))
            if scl_label >= 2:
                scl_label = 2
            example = SemEvalExample(str(i), words, term_texts, starts, ends, new_polarities, scl_label)
...
if __name__ == "__main__":
    ...
    demo_path = "../data/absa/rest_total_train.txt"
    tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased", cache_dir="cache_bert")
    demo_set = read_absa_data(demo_path)
    print(demo_set[0])
    demo_examples = convert_absa_data(demo_set, False)
    print(demo_examples[0])
    demo_features = convert_examples_to_features(demo_examples,tokenizer,85,False,logger)
    ...
    # print("input_ids:", demo_features[0].input_ids, type(demo_features[0].input_ids))
    # print("input_mask:", demo_features[0].input_mask, type(demo_features[0].input_mask))
    # print("aspect_num:", demo_features[0].aspect_num, type(demo_features[0].aspect_num))
    # print("scl_label:", demo_features[0].scl_label, type(demo_features[0].scl_label))